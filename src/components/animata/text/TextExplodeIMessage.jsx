...
const createExplosion = ({ index, total }) => {
...
const splitText = (text) => String(text).split(/(?:)/u);
...
export default function TextExplodeIMessage({
  ...
  const animateSequence = useCallback(async () => {
    await Promise.all([
      controls.start('shrink', { duration: 1, ease: 'easeOut' }),
      controls.start('jitter', { delay: 0.1 }),
    ]);
  ...
  return (
    <motion.div
      ...
      className={`flex items-center justify-center text-3xl tracking-normal ${className}`}
    >
